...
.data
	...
	errorSregLost: .asciz "Some s# registers not preserved.\n"
	newline: .asciz " \n" 	
...
.text
        ...
        #####################################
    li s0,  0xdffffff0 # For testing
    li s8,  0xdffffff1 # For testing
    li s9,  0xdffffff2 # For testing
    li s10, 0xdffffff3 # For testing
    li s11, 0xdffffff4 # For testing
	...
	fileRead(a0, 0xffff0000) # read from file
...
next_record:
	...
	beqz a0, EXIT  #*!# if no valid pointer EXIT
	...
	beqz a0, EXIT   #*!# if not valid pointer EXIT
	print_record(minPrompt)

    # Check that these s# registers did not change.
    li  t1, 0xdffffff0 # testing s0
    bne t1, s0, ERROR_S_REG
    li  t1, 0xdffffff1 # testing s8
    bne t1, s8, ERROR_S_REG
    li  t1, 0xdffffff2 # testing s9
    bne t1, s9, ERROR_S_REG
    li  t1, 0xdffffff3 # testing s10
    bne t1, s10, ERROR_S_REG
    li  t1, 0xdffffff4 # testing s11
    bne t1, s11, ERROR_S_REG
    b EXIT
ERROR_S_REG:
	print_str(errorSregLost)
EXIT:
	exit
